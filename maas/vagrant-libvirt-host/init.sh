...
maas admin subnet update $SUBNET gateway_ip=IP_ADDR_BASE.1
maas admin ipranges create type=dynamic start_ip=IP_ADDR_BASE.200 end_ip=IP_ADDR_BASE.254
maas admin ipranges create type=reserved start_ip=IP_ADDR_BASE.1 end_ip=IP_ADDR_BASE.9
maas admin vlan update $FABRIC_ID $VLAN_TAG dhcp_on=True primary_rack=$PRIMARY_RACK
maas admin maas set-config name=upstream_dns value=8.8.8.8
# Add Libvirt/Virsh as a VM host for MAAS
maas admin vm-hosts create type=virsh power_address=qemu+ssh://HOST_USER@HOST_IP/system
